...
/*
 * flexTimerInit initializes the FlexTimer and starts it
 * 375 count sets interval to 1 ms 
 */
void flexTimerInit(void) {
	
	//as bus clock is set to 12Mhz and FlexTimer prescaler divide factor is set to 32
	//that results in FlexTimer clock of 12 MHz/32 or 0.375 MHz.  If the
	//count used in the FlexTimer modulo register is set to 375, that will
	//result in a FlexTimer interrupt once every 1 ms. */
	flexTimer0Init(375);
	flexTimer0Start();
}

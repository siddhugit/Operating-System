...
enum MAJOR{LED=0,BTN=1,MFS=2,LCD=3,UART=4,APM=5,TTR=6,TSR=7,PIPE=8,NAD=9};
...
static struct device devices[NUM_DEVICES] = {
				...
				pipe_create,pipe_delete,
				pipe_read,pipe_write,pipe_list}
		    	};
...
static const char* PIPE_PATH = "/pipe";
...
static int openFile(const char *name,int fd,char mode)
{
	...
	else if(startsWith(name,PIPE_PATH))
	{
		fname = NULL;
		major = PIPE;
	}
...
{	
	...
	for(j = i + 1; j < NUM_FDS; ++j)
	{
		if(OPEN_FDS[j].stream == NULL)
		{
			fds[1] = j;
			break;
		}
	}
	void *stream = devices[PIPE].open_func("/pipe",'w');
	for(i = 0; i < 2; ++i)
	{
		OPEN_FDS[fds[i]].stream = stream;
		OPEN_FDS[fds[i]].major = PIPE;
		OPEN_FDS[fds[i]].name = "/pipe";
		OPEN_FDS[fds[i]].mode = (i == 0 ? READ : WRITE);
	}
	return 0;
}
...
void vfs_close(int fd)
{
	...
	else if(fd == getConsoleFD())
	{
		return;//never close console
	}
